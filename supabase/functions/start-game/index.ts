...
serve(async (req) => {
  ...
  try {
    ...
    if (room.host_id !== user.id) return new Response(JSON.stringify({ error: 'Not host' }), { status: 403, headers: corsHeaders });

    await supabase.from('rooms').update({
      status: 'playing', word_queue, current_index: 0,
    }).eq('id', room_id);
    ...
    await supabase.channel(`room:${room_id}`).send({
      ...
      payload: {
        ...
        config: {
          ...
          timerSeconds: room.timer_seconds, difficulty: room.difficulty,
        }
      }
    });
